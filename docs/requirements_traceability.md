...
|------------|---------|--------------------------------------------------|--------------------------------------------------|-----------------------------------------|
| REQ-PER-01 | SG1     | 障害物検出精度 ≥ 90%                              | `perception.py`, `tracking.hpp`                  | `tests/test_modules.py` (perception)    |
| REQ-SAF-01 | SG1     | 停止距離保証: 既知障害物手前で必ず停車            | `guardrail.py` (RSS/TTC), `speed_governor.hpp`   | `tests/test_safety.py` (case 1, 2)     |
| REQ-SAF-02 | SG2     | 横断障害物予測: T_horizon 内に衝突を事前回避      | `speed_governor.hpp` (定速予測)                  | `tests/test_safety.py` (case 2)        |
| REQ-SAF-03 | SG2     | 緊急ブレーキ ≤ 0.5 s 応答                         | `guardrail.py` (veto+brake)                      | `tests/test_modules.py` (guardrail)    |
...
- REQ-CTL-01: LQR クロストラック誤差 3.12e-31 m (直線), MPC 速度維持誤差 0 m/s
