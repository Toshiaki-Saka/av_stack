...
回避が存在しないため — **縦方向の制動**である。条件 0 と 1 は誤検知対策であり、安全上の
クレジットは持たない。可用性を守るために存在する。

なお $d_{lat}$ は、現実的な割り込み速度において $\text{LANE}/2$ を超える必要がある
($vy = 2$ m/s で $d_{lat} = 3.82$ m、対して $\text{LANE}/2 = 1.75$ m)。さもなければ、この
チェックはエージェントが自車線内の先行車になってからでないと発火できず、その時点では縦方向
チェックがすでに発火しているため、横方向ブランチは何も上乗せしない。
