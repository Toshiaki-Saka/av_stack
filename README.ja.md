...
| ガードレール（割り込みシナリオ） | 横方向 RSS が縦方向単独より $0.5$ s 早く反応（$1.2$ s 対 $1.7$ s）、余裕 $+1.9$ m |
...
|---|---|---|---|
| `pipeline`（既定） | 急制動 8 m/s² | 両方 | *シナリオ名ではない* — `hard_brake` を4通り（正常/故障 × guardrail ON/OFF）走らせて比較を出力し、故障+guardrail の1本をアニメーション |
| `animation` | 急制動 + 割り込み | — | 純Pythonリアルタイムアニメーション（C++ ビルド不要） |
| `hard_brake` | 急制動 8 m/s² | 正常 | 最大減速度でもプランナーが自力で車間を維持。guardrail は沈黙 |
...
| `cut_in` | 左隣レーンからの割り込み | 故障 | プランナーが割り込みを無視、隣接車が自車線に入る前に横方向 RSS が発火 |
